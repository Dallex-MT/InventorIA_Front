	...
	return (
		<>
			...
			<CommandDialog open={open} onOpenChange={setOpen} modal={false}>
				<CommandInput placeholder={t("common.searchText")} value={searchQuery} onValueChange={setSearchQuery} />
				<CommandList className="min-h-[400px]">
					<CommandEmpty>{t("sys.nav.search.no_results")}</CommandEmpty>
					<CommandGroup heading={t("sys.nav.search.navigations")}>
				...
				<div className="flex flex-wrap text-text-primary p-2 justify-end gap-2">
					<div className="flex items-center gap-1">
						...
						<Badge variant="info">↓</Badge>
						<Text variant="caption">{t("sys.nav.search.to_navigate")}</Text>
					</div>
					<div className="flex items-center gap-1">
						<Badge variant="info">↵</Badge>
						<Text variant="caption">{t("sys.nav.search.to_select")}</Text>
					</div>
					<div className="flex items-center gap-1">
						<Badge variant="info">ESC</Badge>
						<Text variant="caption">{t("sys.nav.search.to_close")}</Text>
					</div>
				</div>
			</CommandDialog>
		</>
